...
import { Grid3X3, Bookmark, Settings, MessageCircle, Camera, Heart } from "lucide-react"
...
export function ProfileContent({
  ...
  async function fetchProfileData() {
    const [followCheck, followers, following, userPosts, userStories] = await Promise.all([
      ...
      supabase
        .from("posts")
        .select("*, profiles!posts_user_id_fkey(*), post_likes (id), comments (id)")
    ...
    setHasStories((userStories.data?.length || 0) > 0)

    if (userPosts.data) {
      const postsWithCounts = userPosts.data.map((post: any) => ({
        ...post,
    ...
    if (isOwnProfile) {
      const { data: saved } = await supabase
        .from("saved_posts")
        .select("post_id, posts(*, profiles!posts_user_id_fkey(*), post_likes(id), comments(id))")
        .eq("user_id", currentUserId)
        .order("created_at", { ascending: false })

      if (saved) {
        const savedWithCounts = saved
          .filter((s: any) => s.posts)
          .map((s: any) => ({
            ...s.posts,
            likes_count: s.posts.post_likes?.length || 0,
            comments_count: s.posts.comments?.length || 0,
          }))
      ...
      if (liked) {
        const likedWithCounts = liked
          .filter((l: any) => l.posts)
          .map((l: any) => ({
            ...l.posts,
            likes_count: l.posts.post_likes?.length || 0,
            comments_count: l.posts.comments?.length || 0,
          }))
  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      <div className="flex flex-col items-center gap-4 py-4 sm:py-6 md:flex-row md:items-start md:gap-8 animate-in fade-in slide-in-from-top-4 duration-500">
        {/* Avatar section */}
        <div className="relative group shrink-0 transition-all duration-300 hover:scale-105">
          <div
            className={`p-0.5 sm:p-1 rounded-full shadow-xl ${
              hasStories
          ...
          >
            <Avatar className="size-20 sm:size-28 md:size-36 border-2 sm:border-4 border-background">
              ...
              <AvatarFallback className="text-2xl sm:text-4xl md:text-5xl bg-linear-to-br from-primary/80 to-primary">
          ...
          {isOwnProfile && (
            <button
              ...
              className="absolute bottom-0 right-0 p-1.5 sm:p-2 rounded-full bg-linear-to-r from-blue-500 to-purple-500 text-primary-foreground shadow-lg opacity-0 group-hover:opacity-100 transition-all duration-200 hover:shadow-xl hover:scale-110"
            >
        ...
        <div className="flex-1 w-full text-center md:text-left">
          ...
          <div className="flex flex-col items-center gap-2 sm:gap-3 mb-3 sm:mb-4 md:flex-row md:items-center">
            <h1 className="text-lg sm:text-xl md:text-2xl font-semibold truncate max-w-full bg-clip-text text-transparent bg-linear-to-r from-blue-500 via-purple-500 to-pink-500">{profile.username}</h1>
              ...
              {isOwnProfile ? (
                <>
                  <Button
                    ...
                    size="sm"
                    onClick={() => setShowEditProfile(true)}
                    className="rounded-lg text-xs sm:text-sm h-8 sm:h-9 shadow-md hover:shadow-lg transition-all duration-200"
                  >
                    Edit profile
                  </Button>
                  <Link href="/settings">
                    <Button variant="ghost" size="icon" className="rounded-lg size-8 sm:size-9 hover:bg-primary/10">
                ...
                <>
                  <Button
                    ...
                    className="rounded-lg min-w-20 sm:min-w-24 text-xs sm:text-sm h-8 sm:h-9 shadow-md hover:shadow-lg transition-all duration-200"
                  >
                    {isFollowing ? "Following" : "Follow"}
                  </Button>
                  <Link href={`/messages?user=${profile.id}`}>
                    <Button variant="secondary" size="sm" className="rounded-lg text-xs sm:text-sm h-8 sm:h-9 shadow-md hover:shadow-lg transition-all duration-200">
                      Message
                    </Button>
                  </Link>
                </>
          ...
          <div className="flex justify-center gap-4 sm:gap-6 md:gap-8 mb-3 sm:mb-4 md:justify-start">
            <div className="text-center md:text-left">
              <span className="font-bold text-base sm:text-lg bg-clip-text text-transparent bg-linear-to-r from-blue-500 via-purple-500 to-pink-500">{posts.length}</span>
            ...
            <button onClick={() => setShowFollowers(true)} className="text-center md:text-left hover:opacity-70">
              <span className="font-bold text-base sm:text-lg bg-clip-text text-transparent bg-linear-to-r from-blue-500 via-purple-500 to-pink-500">{followerCount}</span>
            ...
            <button onClick={() => setShowFollowing(true)} className="text-center md:text-left hover:opacity-70">
              <span className="font-bold text-base sm:text-lg bg-clip-text text-transparent bg-linear-to-r from-blue-500 via-purple-500 to-pink-500">{followingCount}</span>
          ...
          <div className="space-y-0.5 sm:space-y-1 px-4 md:px-0">
            {profile.display_name && <p className="font-semibold text-sm sm:text-base bg-clip-text text-transparent bg-linear-to-r from-blue-500 via-purple-500 to-pink-500">{profile.display_name}</p>}
            {profile.bio && (
